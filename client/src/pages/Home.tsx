...
const gallery = [
  { image: "/glass-facade_1281cb63_3c523186.jpg", kicker: "ضوء · خطوط · اتزان", title: "واجهات زجاجية واستركشر", category: "زجاج وواجهات", text: "حل واجهة يوازن بين حضور الزجاج وانضباط الخطوط؛ ليصل الضوء إلى الداخل ويحافظ على قراءة معمارية واضحة." },
  { image: "/mathabah-architectural-texture_423c537d_e9b373de.webp", kicker: "خامة · عمق · هدوء", title: "تفاصيل الديكور الداخلي", category: "ديكورات داخلية", text: "جدار داخلي بخامة دافئة وإضاءة موزعة بعناية؛ يمنح المساحة نقطة تركيز عملية." },
  { image: "/staircase-detail_36a86d04_67116930.jpg", kicker: "دقة · حركة · ضوء", title: "سلالم وتفاصيل إنشائية", category: "تفصيل وألمنيوم", text: "تفاصيل إنشائية تُعامل كجزء من التصميم، لتبقى الحركة آمنة وواضحة." },
  { image: "/reception_32a5bffe_05c6fa3b.jpg", kicker: "تصميم · تنفيذ · حضور", title: "استقبال ومساحات عمل", category: "ديكورات داخلية", text: "ترتيب استقبال يوجّه الحركة ويترك انطباعًا منظمًا منذ اللحظة الأولى." },
  { image: "/glass-office_ff88266f_aac6aeb9.jpg", kicker: "ظل · أداء · واجهة", title: "واجهات وخامات معاصرة", category: "زجاج وواجهات", text: "تكوين خارجي يوضح كيف يمكن للخامة والظل أن يقدما واجهة هادئة ذات حضور." },
  { image: "/built-in-lounge_7d8805b0_f1ed0f5e.jpg", kicker: "تفصيل · نجارة · دفء", title: "أعمال خشبية مخصصة", category: "تفصيل وألمنيوم", text: "تفصيل خشبي مخصص يضيف دفئًا وتنظيمًا إلى المكان." },
];

const services = [
  { no: "01", category: "زجاج وواجهات", image: gallery[0].image, title: "أعمال الزجاج والسكريت", label: "شفافية محسوبة", text: "واجهات زجاجية، كبائن شاور، درابزينات وواجهات استركشر تُنفذ بمواصفات واضحة.", href: "/services/glass" },
  { no: "02", category: "ديكورات داخلية", image: gallery[1].image, title: "التصميم والديكور الداخلي", label: "تفصيل يخصكم", text: "حلول داخلية للمنازل والمكاتب، من اختيار الخامة إلى تركيب التفاصيل.", href: "/services/interior-decor" },
  { no: "03", category: "زجاج وواجهات", image: gallery[4].image, title: "الألمنيوم والواجهات الخارجية", label: "أداء يدوم", text: "نوافذ وأبواب وتكسيات ألمنيوم وكلادينج تُصمم لأداء معماري متوازن.", href: "/services/aluminum" },
];
...
export default function Home() {
  ...
  const [adminUnlocked, setAdminOpenFields] = useState(false);
  ...
  useEffect(() => {
    const onKey = (event: KeyboardEvent) => { 
      ...
      if (event.key === "Escape") { 
        closeLightbox(); setPromoOpen(false); setMenuOpen(false); setAdminOpen(false);
      } 
    };
  ...
  const verifyAdmin = () => {
    if (adminPass === "7788") {
      setAdminOpenFields(true);
    } else {
      setToast("الرمز غير صحيح");
    }
  };
  useEffect(() => {
  ...
  return <div dir="rtl" className={`site-shell theme-${theme}`} id="home">
    ...
    <header className={`site-header ${scrolled ? "is-scrolled" : ""}`}>
      <a href="#home" className="brand" aria-label="مثابة، العودة إلى الرئيسية"><img src={"/mathabah-logo-transparent_2365ed67.png"} alt="مثابة — نبني ما يُعتمد عليه" /></a>
    ...
    <main>
      <section className="hero section-grid"><div className="hero-copy"><div className="mini-label">مؤسسة مثابة <span>زجاج · ألمنيوم · نجارة</span></div><h1>نحوّل رؤيتكم إلى<br /><em>تفاصيل</em> تُعتمد.</h1><p>نصمم وننفذ أعمال الزجاج والسكريت، الألمنيوم، والديكورات والنجارة الخشبية من الفكرة حتى التركيب، بمواصفة واضحة تناسب المكان والاستخدام.</p><div className="hero-actions"><a className="button button-copper" href="#contact">ابدأوا الحديث <ArrowDownLeft size={16} /></a><a className="button button-ghost" href="#work">شاهدوا أعمالنا <ArrowDownLeft size={16} /></a></div><div className="hero-note"><Check size={15} /> زجاج واضح · ألمنيوم عملي · نجارة دقيقة</div></div><div className="hero-visual"><img src={gallery[1].image} alt="تفاصيل ديكور ونجارة خشبية معاصرة" /><div className="image-caption"><span>01</span><strong>المكان يبدأ من الصورة</strong><small>خامات تُختار لتخدم المكان</small></div><div className="watermark">M<br /><small>GLASS · ALUMINUM<br />WOODWORK</small></div></div></section>

      <section id="expertise" className="section dark-section"><div className="section-grid intro-grid"><div><SectionHeading eyebrow="01  منهج مثابة" accent="ونتيجة واضحة.">قرارات مدروسة</SectionHeading><p className="lead">نبدأ بفهم الاستخدام والميزانية وشخصية المكان، ثم نختار بين الزجاج والألمنيوم والخشب بما يخدم المشروع. هدفنا تنفيذ متوازن في شكله، عملي في أدائه، وواضح في كل مرحلة.</p></div><div className="process-list"><div><b>01</b><span>فهم الاحتياج</span></div><div><b>02</b><span>اختيار الزجاج أو الألمنيوم أو الخشب</span></div><div><b>03</b><span>ضبط التصنيع والتركيب</span></div></div></div></section>

      <section className="section services-section"><SectionHeading eyebrow="02  مجالات الخبرة" accent="على معرفة.">حلول تُبنى</SectionHeading><p className="section-intro">من الفكرة الأولى حتى اللمسة الأخيرة، نضع الخبرة في مكانها الصحيح: خلف التفاصيل، لا فوقها.</p><div className="services-grid">{services.map((service, index) => <button className={`service-card ${activeService === index ? "active" : ""}`} key={service.no} onClick={() => { setActiveService(index); selectFilter(service.category); window.setTimeout(() => document.getElementById("work")?.scrollIntoView({ behavior: "smooth", block: "start" }), 40); }}><img src={service.image} alt={service.title} /><div className="service-overlay" /><span className="service-number">{service.no}</span><div className="service-content"><span>{service.label}</span><h3>{service.title}</h3><p>{service.text}</p><b>استكشف المجال <ArrowDownLeft size={16} /></b></div></button>)}</div><div className="service-directory" aria-label="صفحات الخدمات"><span>صفحات الخدمات:</span><a href="/services/glass">الزجاج والسكريت</a><a href="/services/interior-decor">الديكور الداخلي</a><a href="/services/woodwork">الأعمال الخشبية والنجارة</a><a href="/services/aluminum">الألمنيوم والواجهات</a></div></section>
    ...
    <footer><div className="footer-top"><img src={"/mathabah-logo-transparent_2365ed67.png"} alt="مثابة" /><p>زجاج وألمنيوم ونجارة وديكورات خشبية<br />تُنفذ بتفاصيل واضحة.</p><div className="footer-links"><span>روابط سريعة</span><a href="#expertise">خبرتنا</a><a href="#work">سجل الأعمال</a><a href="#identity">هويتنا</a></div><div className="socials"><span>قنوات التواصل</span>{socialChannels.map((channel) => <button key={channel.key} onClick={() => comingSoon(channel.name)} aria-label={`${channel.name} — ستتوفر القناة قريبًا`}><SocialGlyph type={channel.key} /> {channel.name}</button>)}</div></div><div className="footer-bottom"><span>© 2026 مؤسسة مثابة</span><span>نبني ما يُعتمد عليه</span></div></footer>
    ...
    {adminOpen && (
      <div className="modal-backdrop" style={{zIndex: 20000}}>
        <div className="notice-modal" style={{padding: '30px', textAlign: 'center'}}>
          <button className="modal-close" onClick={() => setAdminOpen(false)}><X /></button>
          <h2 style={{fontSize: '1.5rem', marginBottom: '20px'}}>لوحة التحكم الإدارية</h2>
          {!adminUnlocked ? (
            <div style={{display: 'flex', flexDirection: 'column', gap: '15px'}}>
              <input 
                type="password" 
                placeholder="أدخل رمز الدخول" 
                className="brief-form input" 
                style={{textAlign: 'center', background: '#f5f5f5', border: '1px solid #ddd', color: '#333', padding: '10px'}}
                value={adminPass}
                onChange={(e) => setAdminPass(e.target.value)}
              />
              <button className="button button-copper" onClick={verifyAdmin} style={{width: '100%', justifyContent: 'center'}}>تحقق</button>
            </div>
          ) : (
            <div style={{textAlign: 'right', fontSize: '0.9rem', color: '#555'}}>
              <p>تم تفعيل وضع التعديل. يمكنك الآن تحديث الروابط من ملف الإعدادات.</p>
              <button className="button button-ghost" onClick={() => {setAdminOpen(false); setAdminOpenFields(false); setAdminPass("");}} style={{width: '100%', marginTop: '15px', justifyContent: 'center'}}>إغلاق</button>
            </div>
          )}
        </div>
      </div>
